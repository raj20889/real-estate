...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($name) || empty($email) || empty($phone) || empty($password)) {
        ...
        if ($result->num_rows > 0) {
            ...
            // Insert new seller
            $stmt = $conn->prepare("INSERT INTO sellers (name, email, phone, password, created_at) VALUES (?, ?, ?, ?, NOW())");
            $stmt->bind_param("ssss", $name, $email, $phone, $hashed_password);

            if ($stmt->execute()) {
                echo "<script>
                    alert('Success: Registration successful. Redirecting to login...');
...
<head>
    ...
    <style>
        body {
            ...
            margin: 0;
            padding: 0;
            background: url('images/seller1.jpg') no-repeat center center/cover;
            display: flex;
...
<body>
    <div class="form-container">
        ...
        <form action="" method="POST" id="registrationForm">
            ...
            <input type="tel" id="phone" name="phone" pattern="\+[0-9]{1,3}[0-9]{4,14}" 
                   required placeholder="e.g., +919876543210">
