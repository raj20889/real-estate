...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $conn->prepare("INSERT INTO inquiries (property_id, user_id, inquiry_date, message, created_at) VALUES (?, ?, ?, ?, NOW())");
    $stmt->bind_param("iiss", $property_id, $user_id, $inquiry_date, $message);
    
    if ($stmt->execute()) {
        $success_message = "Inquiry submitted successfully!";
    } else {
        $success_message = "Error submitting inquiry.";
    }
...
<body class="flex items-center justify-center min-h-screen p-4">
    <div class="form-container p-8 rounded-xl w-full max-w-md">
        ...
        <form method="POST" action="" class="space-y-5">
            ...
            <button type="submit" class="w-full bg-green-500 hover:bg-green-600 text-white font-medium py-3 px-4 rounded-lg shadow-md transition-all duration-300 transform hover:scale-[1.02]">
                Submit Enquiry
            </button>
        </form>
    ...
    <div id="popup" class="success-modal fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
        <div class="bg-white p-8 rounded-xl shadow-2xl w-96 text-center">
            ...
            <h3 class="text-xl font-semibold text-gray-800 mb-2"><?php echo $success_message; ?></h3>
